/* ═══════════════════════════════════════════════════════════
   CHRONOS CAPSULE — archive.js (نهائي v5)
   Skeleton + Empty States + Likes + Ratings + Schema.org
   ═══════════════════════════════════════════════════════════ */
(function(){
    'use strict';
    ...
    function getSkeletonHTML() {
        ...
        return `<div class="skeleton-list" role="status" aria-label="${label}">${item.repeat(4)}</div>`;
    }
    ...
    function getEmptyStateHTML(type) {
        ...
        const config = {
            'no-results': {
                ...
                desc: isAr ? 'لم يُعثر على أي رسالة تطابق الفلاتر التي اخترتها. جرّب تغيير الفلاتر أو البحث بكلمات أخرى.'
            ...
            'no-capsules': {
                ...
                desc: isAr ? 'الفضاء ينتظر رسالتك. ابدأ رحلتك عبر الزمن الآن — اكتب كبسولة، واختر متى تصل.'
                            : 'Space awaits your message. Start your time travel now — write a capsule and choose when it arrives.',
                cta: { href: 'index.html', label: isAr ? '🚀 إطلاق أول كبسولة' : '🚀 Launch First Capsule' },
                hint: isAr ? 'كل كبسولة تبقى في الفضاء حتى موعدها — ستحصل على مفتاح سري لمتابعتها'
                            : 'Every capsule stays in space until its time — you\'ll get a secret key to track it'
            },
            'error': {
                icon: '⚠️',
                title: isAr ? 'حدث خطأ' : 'Something Went Wrong',
                desc: isAr ? 'تعذّر تحميل الرسائل من الفضاء. تحقق من اتصالك بالإنترنت وحاول مجددًا.'
                            : 'Failed to load messages from space. Check your internet connection and try again.',
                cta: { href: 'javascript:location.reload()', label: isAr ? '🔄 حاول مجددًا' : '🔄 Try Again' },
                hint: null
            }
        };

        const c = config[type] || config['no-results'];
        const ctaHTML = c.cta ? `<div class="empty-state-actions"><a href="${c.cta.href}" class="empty-state-cta">${c.cta.label}</a></div>` : '';
        const hintHTML = c.hint ? `<div class="empty-state-hint"><span class="empty-state-hint-icon">💡</span><span>${c.hint}</span></div>` : '';
    ...
    async function fetchArchive(reset) {
        reset = !!reset;
        if (!sb) { showEmpty($('archiveList'), 'error'); return; }
        if (isLoading) return;
        isLoading = true;
        ...
        if (error) { if (reset) showEmpty(list, 'error'); return; }
        ...
        if (r.id) {
            ...
            ld.textContent = JSON.stringify({
                ...
                "publisher": {
                    ...
                    "logo": {
                        "@type": "ImageObject",
                        "url": "https://chronoscapsule.vercel.app/icon-512.svg",
                        "width": 512,
                        "height": 512
                    }
                },
